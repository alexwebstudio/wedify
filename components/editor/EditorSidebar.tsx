        ...
        {tabs.map((tb) => (
          <button
            ...
            className={`flex-1 flex flex-col items-center gap-1 py-2.5 text-[9.5px] font-medium transition-colors ${
              tab === tb.key ? 'text-[#6E2B34] border-b-2 border-[#6E2B34] bg-[#6E2B34]/5' : 'text-ink-400 hover:text-ink-600'
            }`}
          >
            {tb.icon}
            {tb.label}
        ...
        <AnimatePresence mode="wait">
          ...
          {tab === 'blocks' && (
            <motion.div key="blocks" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="p-3">
              <button
                ...
                style={canAddBlocks
                  ? { background: 'linear-gradient(135deg,#6E2B34,#4A1A22)', color: '#fff' }
                  : { background: '#EFEAE0', color: '#6F655B' }}
              >
          ...
          {tab === 'data' && (
            <motion.div key="data" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="p-3 space-y-3">
              <p className="text-[11px] text-ink-400 leading-relaxed">Общие данные подставляются во все блоки сразу. Изменения применяются, когда вы уходите из поля.</p>
              {([
                ...
                <div key={key}>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 rounded-lg bg-paper-2 text-sm text-[#16130F] outline-none focus:ring-2 focus:ring-[#6E2B34]/30 transition"
                  />
                </div>
              ))}
              ...
              <div className="pt-3 mt-1 border-t border-paper-3">
                ...
                <p className="text-[11px] text-ink-400 mb-2 leading-snug">Если задать код — гости не откроют сайт без него. Пусто — доступ свободный.</p>
                <input
                  value={project.music.accessPin || ''}
                  placeholder="Например, 2026"
                  maxLength={12}
                  onChange={(e) => onUpdate({ music: { ...project.music, accessPin: e.target.value } })}
                  className="w-full px-3 py-2 rounded-lg bg-paper-2 text-sm text-[#16130F] tracking-[0.25em] text-center outline-none focus:ring-2 focus:ring-[#6E2B34]/30"
                />
          ...
          {tab === 'style' && (
            <motion.div key="style" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="p-3 space-y-2">
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between mb-2">
                      ...
                      {active && <Check size={14} className="text-[#6E2B34]" />}
                    </div>
              ...
              <div className="pt-3 mt-1 border-t border-paper-3">
                  ...
                  {BUTTON_SHAPES.map((o) => {
                    ...
                    return (
                      <button key={o.v}
                        ...
                        className={`flex-1 py-2 text-[11px] font-medium transition-all border ${active ? 'bg-[#16130F] text-white border-[#16130F]' : 'bg-white text-ink-400 border-paper-3 hover:border-[#6E2B34]/50'}`}
                ...
                <p className="text-[10px] uppercase tracking-widest text-ink-400 mb-2 px-1">Форма изображений</p>
                  ...
                  {IMAGE_SHAPES.map((o) => {
                    ...
                    return (
                      <button key={o.v}
                        ...
                        className={`flex items-center gap-2 py-2 px-2.5 text-[11px] font-medium transition-all border rounded-xl ${active ? 'border-[#6E2B34] bg-[#6E2B34]/8 text-[#4A1A22]' : 'border-paper-3 text-ink-400 hover:border-[#6E2B34]/50'}`}>
                        <span className="w-4 h-4 border-2 flex-shrink-0" style={{ borderRadius: r, borderColor: active ? '#6E2B34' : '#c9c2b8' }} />
                        {o.label}
                      </button>
                    )
                  })}
                  ...
                  {COLOR_PRESETS.map((preset) => {
                    ...
                    return (
                      <button
                        ...
                        className={`p-2.5 rounded-xl border text-left transition-all ${isActive ? 'ring-2 ring-[#6E2B34]' : 'hover:border-[#6E2B34]/50'}`}
                      ...
                      >
                        <div className="flex gap-1 mb-1.5">
                          ...
                          {isActive && <Check size={12} className="ml-auto text-[#6E2B34]" />}
                        </div>
          ...
          {tab === 'fonts' && (
            <motion.div key="fonts" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="p-3 space-y-2">
              <p className="text-[10px] uppercase tracking-widest text-ink-400 mb-2 px-1">{WEDDING_FONTS.length} свадебных шрифтов</p>
              {WEDDING_FONTS.map((f) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full p-3 rounded-xl border text-left transition-all ${
                      active ? 'border-[#6E2B34] bg-[#6E2B34]/5 ring-1 ring-[#6E2B34]/30' : 'border-paper-3 hover:border-[#6E2B34]/40'
                    }`}
                  >
                    <div className="flex items-center justify-between mb-1">
                      ...
                      {active && <Check size={12} className="text-[#6E2B34]" />}
                    </div>
                    <p className="text-[#16130F] leading-tight truncate" style={{ fontFamily: fontFamilyValue(f.heading), fontSize: f.kind === 'cursive' ? 26 : 22, fontWeight: 400 }}>
                      Айгерім &amp; Дамир
                    </p>
                  </button>
                )
          ...
          {tab === 'music' && (
            <motion.div key="music" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="p-3 space-y-3">
              ...
              {project.music.url ? (
                <div className="p-3 rounded-xl bg-[#6E2B34]/10 border border-[#6E2B34]/20">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-sm font-medium text-[#16130F] truncate max-w-[80%]">{project.music.title || 'Музыка'}</span>
              ...
              <div className="flex items-center justify-between p-3 rounded-xl bg-paper-2">
                <div>
                  <p className="text-sm font-medium text-[#16130F]">Автовоспроизведение</p>
              ...
              <div className="pt-2 border-t border-paper-3">
                <div className="flex items-center justify-between px-1 mb-2">
                  ...
                  <span className="text-[10px] px-1.5 py-0.5 rounded-full" style={{ background: '#6E2B3422', color: '#4A1A22' }}>{libraryForPlan(plan).length} треков</span>
                </div>
                {plan === 'start' ? (
                  <p className="text-[11px] text-ink-400 px-1 leading-snug">Библиотека доступна на тарифе Стандарт (10) и Премиум (30). Загрузите свой файл выше.</p>
                ) : (
                  <>
                    <p className="text-[11px] text-ink-400 px-1 leading-snug mb-2">Выберите трек. Файлы подставляются вашими лицензированными копиями (загрузка в Storage).</p>
                    <div className="max-h-60 overflow-y-auto space-y-1 pr-1" data-lenis-prevent>
                      {libraryForPlan(plan).map((tr) => {
                        ...
                        return (
                          <div key={tr.id} className={`flex items-center gap-2 p-2 rounded-lg ${active ? 'bg-[#6E2B34]/12' : 'hover:bg-paper-2'}`}>
                            <button
                              ...
                              style={{ background: tr.url ? '#6E2B34' : '#CBB' }} title={tr.url ? 'Прослушать' : 'Файл не задан'}>
                              {musicPreview === tr.id ? '❚❚' : '▶'}
                            </button>
...
function BlockRow({ block, onToggle, onDuplicate, onDelete }: {
  ...
  return (
    <Reorder.Item
      ...
      style={{ borderColor: block.enabled ? 'rgba(110, 43, 52,0.25)' : '#f0f0f0', background: block.enabled ? '#FBF8F4' : '#fafafa' }}
    ...
    >
      ...
      <span className={`text-[13px] font-medium flex-1 min-w-0 truncate ${block.enabled ? 'text-[#16130F]' : 'text-ink-400'}`}>
        {blockTitle(block)}
      </span>
    ...
    <div className="p-2.5 rounded-xl border border-paper-3">
      <div className="flex items-center gap-2 mb-2">
        ...
        <input
          ...
          onKeyDown={(e) => { if (e.key === 'Enter') (e.target as HTMLInputElement).blur() }}
          className="w-[74px] px-2 py-1 rounded-lg bg-paper-2 text-[12px] font-mono text-ink-600 outline-none focus:ring-2 focus:ring-[#6E2B34]/30 uppercase"
        />
      </div>
